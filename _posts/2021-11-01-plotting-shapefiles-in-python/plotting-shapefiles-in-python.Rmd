...
Firstly, install `Anaconda` with instructions [here.](https://docs.conda.io/projects/conda/en/latest/user-guide/install/macos.html) To minimize the packages you're installing, go ahead with `miniconda`. This can be done through the commandline. Both `Anaconda` or `Miniconda` will work fine whether you have a PC or Mac.

Next, install the IDE [VSCode](https://code.visualstudio.com/). 
...
Create a new conda environment with `conda create -n EnvName` and tag on `python=3.10` to the end if you want to specify a certain python version. Activate the environment with `conda activate EnvName`. Don't install any packages into your base environment. If you do this by accident, you can always reset it.
...
The fig size can be whatever you want. You have finer control over the degree of zoom of the map with the arguments `xlim()` and `ylim()`, anyway. These polygons are just floating in space, so lets add a basemap to give us geographical context:  
