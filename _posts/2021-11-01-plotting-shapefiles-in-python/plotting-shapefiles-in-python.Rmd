...
First things first, if you're using Jupyter Notebooks to code in Python, you're gonna have to install Python and Anaconda Navigator and install `Anaconda` into your home directory or an environment (I recommend making an environment!). (Note: there are plenty of other IDE's for python, such as [visual studio code](https://code.visualstudio.com/), which I adopted later in my data science career).
