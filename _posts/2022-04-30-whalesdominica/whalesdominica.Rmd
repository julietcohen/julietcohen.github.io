---
title: "Whale Migration & Vessel Routes in the Caribbean"
description: |
  Whales migrate through seascapes that are heavily utilied by humans. Perhaps a reduction in vessel speed help reduce ship-whale collisions. Using spatial tools in Python, we investigate the marine overlap of these two components and calculate what speed reduction would mean for those vessels.
author:
  - name: Juliet Cohen
    url: {}
date: 2022-04-30
...
In this anaysis in Python, we aim to determine the spatial outline of a speed reduction zone for boats in Dominica, an island in the Caribbean. Furthermore we calculate the effect of this speed reduction zone on the local marine traffic. The reduction zone should ideally encompass the preferred habitat of the local sperm whales, which will be approximated using frequent whale sightings data.

<center>

![A sperm whale, the species that migrated though the seascape around Dominica](outputs/sperm_whale.png){width=60%}
...
![Scotts Head, Dominica](outputs/dominica.png){width=70%}
...
Similarly to what we did above, bootstrap the geometries from the latitude & longitude initially set the CRS to `EPSG 4326`, and we will convert it to the local `EPSG 2002` after we append the geometries:
```python
